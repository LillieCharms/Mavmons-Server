...
export const Moves: {[k: string]: ModdedMoveData} = {
	...
	onPrepareHit(target, source, move) {
		...
		onHit(source) {
			source.side.addSlotCondition(source, 'pitchingchange');
		},
	condition: {
			onSwitchIn(target) {
				if (!target.fainted) {
					this.heal(target.baseMaxhp / 16, target);
					this.add('-heal', target, '[from] move: Pitching Change');
					target.side.removeSlotCondition(target, 'pitchingchange');
				}
			},
		},
	},
	...
	type: "Water",
		condition: {
			duration: 3,
			onSideStart(side, source) {
				this.add('-sidestart', side, 'move: Wavebreaker');
			},
			onSideResidualOrder: 26,
			onSideResidual(side) {
			...
			onSideEnd(side) {
				this.add('-sideend', side, 'move: Wavebreaker');
			},
		},
	},
	inkmine: {
		...
		damageCallback(source, target) {
			return Math.floor(target.baseMaxhp * 0.35);
		},
		secondary: {
			chance: 100,
			boosts: {
   ...
   type: "Psychic",
	...
	overture: {
		...
		category: "Physical",
		name: "Overture",
		shortDesc: "Removes the hazards on the user's side.",
		pp: 15,
		priority: 0,
		flags: {contact: 1, protect: 1, mirror: 1, metronome: 1},
		onAfterHit(target, pokemon, move) {
			if (!move.hasSheerForce) {
				if (pokemon.hp && pokemon.removeVolatile('leechseed')) {
					this.add('-end', pokemon, 'Leech Seed', '[from] move: Overture', '[of] ' + pokemon);
				}
				const sideConditions = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge', 'electricfence'];
				for (const condition of sideConditions) {
					if (pokemon.hp && pokemon.side.removeSideCondition(condition)) {
						this.add('-sideend', pokemon.side, this.dex.conditions.get(condition).name, '[from] move: Overture', '[of] ' + pokemon);
					}
				}
				if (pokemon.hp && pokemon.volatiles['partiallytrapped']) {
					pokemon.removeVolatile('partiallytrapped');
